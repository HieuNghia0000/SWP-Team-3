...
public class ShiftController {
    ...
    public ResponseEntity<Object> createShift(@Valid @RequestBody CreateShiftDto shift, BindingResult errors) {
        ...
        Optional<Shift> createdShift = shiftService.createShift(shift);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Shift> getShiftsById(@PathVariable("id") Integer id) {
        Shift shift = shiftService.getShiftsById(id);
        return new ResponseEntity<>(shift, HttpStatus.OK);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<Shift> updateShifts(@PathVariable("id") Integer id, @RequestBody Shift shift) {
        Shift updatedShift = shiftService.updateShifts(id, shift);
