  ...
  const products = createRouteData(
    async ([ key, id ]) => {
      try {
        ...
        if (data === undefined || data.content === undefined) throw new Error("Order not found");
...
export default function Invoice() {
  ...
  function handlePrintInvoice() {
    ...
    html2pdf().set(pdfOptions).from(pdfData).toPdf().output('blob').then((pdf: Blob) => {
      console.log(pdf);
    ...
    <main class="min-w-fit">
      ...
      <Show when={!data.error && data() !== undefined}>
        <div class="flex flex-row gap-4">
            ...
            <div class="px-10 py-8 bg-white border border-gray-200 rounded-lg overflow-x-auto shadow-sm">
              ...
              <div class="flex items-center justify-between mt-10">
                ...
                <div class="space-y-1">
                  ...
                  <p class="font-semibold text-right">{moment(data()!.orderDate).format("MMM Do YYYY, h:mm:ss a")}</p>
                  <p
                    classList={{
                      "text-orange-400": data()!.paymentStatus === PaymentStatus.PENDING,
                      "text-green-400": data()!.paymentStatus === PaymentStatus.SUCCESS,
                      "text-red-400": data()!.paymentStatus === PaymentStatus.FAILED,
                    }}
                    class="font-bold text-right">{capitalize(data()!.paymentStatus)}</p>
                </div>
              </div>
                ...
                <table class="min-w-full table-fixed">
                  ...
                  <For each={data()?.orderItems}>
                    {(item, index) => (
                      <tr>
                        <td
                          class="px-2.5 pl-[18px] text-base whitespace-nowrap truncate leading-10 border-[#e2e7ee] border-b text-gray-500 font-medium">{item.product.name}</td>
                        <td
                          class="px-2.5 text-base whitespace-nowrap truncate leading-10 border-[#e2e7ee] border-b text-gray-500 font-medium">{item.quantity} pcs
                        </td>
                        <td
                          class="px-2.5 text-base text-right whitespace-nowrap truncate leading-10 border-[#e2e7ee] border-b text-gray-500 font-medium">{formatNumberWithCommas(item.product.price)}&nbsp;₫
                        </td>
                        <td
                          class="px-2.5 text-base text-right whitespace-nowrap truncate leading-10 border-[#e2e7ee] border-b text-gray-500 font-medium">{formatNumberWithCommas(item.product.price * item.quantity)}&nbsp;₫
                        </td>
                      </tr>
                    )}
                  </For>
                  ...
                  <tfoot>
                  <tr>
                    <td colspan="3" class="text-right px-2.5 py-2 font-medium">Subtotal</td>
                    <td class="px-2.5 py-2 font-medium text-right">{formatNumberWithCommas(subTotal())}&nbsp;₫</td>
                  </tr>
                  <tr>
                    <td colspan="3" class="text-right px-2.5 py-2 font-medium">VAT (10%)</td>
                    <td class="px-2.5 py-2 font-medium text-right">{formatNumberWithCommas(subTotal() * 0.1)}&nbsp;₫
                    </td>
                  </tr>
                  <tr>
                    <td colspan="3" class="text-right px-2.5 py-2 font-medium">Grand Total</td>
                    <td
                      class="px-2.5 py-2 font-bold text-right">{formatNumberWithCommas(subTotal() + subTotal() * 0.1)}&nbsp;₫
                    </td>
                  </tr>
                  </tfoot>
                </table>
              ...
              <div class="flex justify-center mt-8">
                <p class="text-gray-500">Should you have inquiries concerning this invoice, please contact <span
                  class="font-bold">Martha</span> on +1 (469) 227 9044</p>
              </div>
            </div>
          </div>
          ...
          <div class="w-72">
            <div
              class="flex flex-col p-6 bg-white border border-gray-200 rounded-lg overflow-x-auto shadow-sm space-y-4">
              <button
