...
const schema: yup.Schema<EditTimesheetForm> = yup.object({
  checkInTime: yup
    .string()
    .required("Invalid check in time"),
  checkOutTime: yup
    .string()
    .required("Invalid check out time"),
...
const EditTimesheetModal: Component = () => {
  ...
  const submit = async (status: TimesheetStatus, event: Event) => {
    ...
    if (moment(formData().checkInTime, "HH:mm").isAfter(moment(formData().checkOutTime, "HH:mm"))) {
          ...
          <div class="w-full flex justify-between items-center gap-2">
            ...
            <div class="flex gap-2 justify-center items-center">
              <button
                type="button"
                disabled={updating.pending}
                onClick={[ submit, timesheet()?.status === TimesheetStatus.REJECTED ? TimesheetStatus.PENDING : TimesheetStatus.REJECTED ]}
                class="py-1.5 px-3 font-semibold border text-sm rounded"
                classList={{
                  "text-white border-red-600 bg-red-500 hover:bg-red-600": timesheet()?.status !== TimesheetStatus.REJECTED,
                  "text-gray-600 border-gray-300 bg-white hover:text-black": timesheet()?.status === TimesheetStatus.REJECTED,
                }}
              >
                {timesheet()?.status === TimesheetStatus.REJECTED ? "Save & Reset" : "Save & Deny"}
              </button>
              <button
                type="button"
                disabled={updating.pending}
                onClick={[ submit, timesheet()?.status === TimesheetStatus.APPROVED ? TimesheetStatus.PENDING : TimesheetStatus.APPROVED ]}
                class="py-1.5 px-3 font-semibold border text-sm rounded"
                classList={{
                  "text-white border-green-600 bg-green-500 hover:bg-green-600": timesheet()?.status !== TimesheetStatus.APPROVED,
                  "text-gray-600 border-gray-300 bg-white hover:text-black": timesheet()?.status === TimesheetStatus.APPROVED,
                }}
              >
                {timesheet()?.status === TimesheetStatus.APPROVED ? "Save & Reset" : "Save & Approve"}
              </button>
              <button
                type="button"
                disabled={updating.pending}
                onClick={[ submit, timesheet()?.status || TimesheetStatus.PENDING ]}
