...
const schema: yup.Schema<Omit<ShiftCoverRequest, "staff" | "shift" | "status">> = yup.object({
  shiftCoverRequestId: yup.number().required("Shift cover request ID is required"),
...
const updateCoverRequest = async (formData: ShiftCoverRequest) => {
  try {
    const { data } = await axios.put<DataResponse<ShiftCoverRequest>>(
      `${getEndPoint()}/shift-cover-requests/update/${formData.shiftCoverRequestId}`, { ...formData }
    )
    console.log(data);
          ...
          <PopupModal.Body>
            <div class="p-5 mb-5 -mx-5 -mt-5 border-b border-gray-200">
              <div
                ...
                classList={{
                  "bg-[#edf2f7] text-black": curCoverRequest()?.shift?.published,
              ...
              >
                <i
                  ...
                  classList={{
                    "bg-blue-500": curCoverRequest()?.shift?.role === Role.CASHIER,
                    "bg-yellow-500": curCoverRequest()?.shift?.role === Role.GUARD,
                    "bg-red-500": curCoverRequest()?.shift?.role === Role.MANAGER,
                    "bg-gray-600": curCoverRequest()?.shift?.role === Role.ADMIN,
                    "bg-gray-400":
                ...
                <p class="ml-3.5 font-normal text-xs tracking-wider">
                  {curCoverRequest()?.shift?.staff?.staffName} - {capitalize(curCoverRequest()?.shift?.role || "")}
                </p>
              </div>
            </div>
              ...
              <div class="flex gap-2">
                <div class="flex-1 py-2.5 flex flex-col gap-1">
                  <Select
                    ...
                    options={staffInfos()!.map((staff) => ({
                      label: staff.staffName,
                      value: staff.staffId,
                    })).filter((staff) => staff.value !== curCoverRequest()?.shift?.staffId)}
            ...
            <div class="w-full flex justify-between items-center gap-2">
              <div class="flex gap-2 justify-center items-center">
                <button
                  ...
                  onClick={[ onDelete, curCoverRequest()?.shiftCoverRequestId || 0 ]}
              ...
              <div class="flex gap-2 justify-center items-center">
                <Show when={user()?.role === Role.ADMIN}>
                  <button
                    type="button"
                    disabled={updating.pending}
                    onClick={[ submit, curCoverRequest()?.status === ShiftCoverRequestStatus.REJECTED ? ShiftCoverRequestStatus.PENDING : ShiftCoverRequestStatus.REJECTED ]}
                    class="py-1.5 px-3 font-semibold border text-sm rounded"
                    classList={{
                      "text-white border-red-600 bg-red-500 hover:bg-red-600": curCoverRequest()?.status !== ShiftCoverRequestStatus.REJECTED,
                      "text-gray-600 border-gray-300 bg-white hover:text-black": curCoverRequest()?.status === ShiftCoverRequestStatus.REJECTED,
                    }}
                  >
                    {curCoverRequest()?.status === ShiftCoverRequestStatus.REJECTED ? "Save & Reset" : "Save & Deny"}
                  </button>
                  <button
                    type="button"
                    disabled={updating.pending}
                    onClick={[ submit, curCoverRequest()?.status === ShiftCoverRequestStatus.APPROVED ? ShiftCoverRequestStatus.PENDING : ShiftCoverRequestStatus.APPROVED ]}
                    class="py-1.5 px-3 font-semibold border text-sm rounded"
                    classList={{
                      "text-white border-green-600 bg-green-500 hover:bg-green-600": curCoverRequest()?.status !== ShiftCoverRequestStatus.APPROVED,
                      "text-gray-600 border-gray-300 bg-white hover:text-black": curCoverRequest()?.status === ShiftCoverRequestStatus.APPROVED,
                    }}
                  >
                    {curCoverRequest()?.status === ShiftCoverRequestStatus.APPROVED ? "Save & Reset" : "Save & Approve"}
                  </button>
                </Show>
                <button
                  type="button"
                  disabled={updating.pending}
                  onClick={[ submit, curCoverRequest()?.status || ShiftCoverRequestStatus.PENDING ]}
